function details () {
  return {
    ...
    Type: 'Video',
    Operation: 'Transcode',
    Description: `[Contains built-in filter] This plugin is specifically made to make files that are chromecast direct streaming compatible. 
    ...
    Version: '1.00',
    Link:
      'https://github.com/HaveAGitGat/Tdarr_Plugins/blob/master/Community/Tdarr_Plugin_vdka_Chromecast_Compatibility.js',
  ...
  for (var i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    for (var i = 0; i < file.ffProbeData.streams.length; i++) {
      ...
      if (streamX.codec_type.toLowerCase() == 'audio') {
        ...
        if (
          streamX.channels == '2' &&
          !['aac', 'mp3', 'opus'].includes(streamX.codec_name)
        ) {
    ...
    for (var i = 0; i < file.ffProbeData.streams.length; i++) {
      ...
      if (streamX.codec_type.toLowerCase() != 'audio') continue
    ...
    if (!response.processFile) {
      response.infoLog +=
        '☑File should be fully chromecast compatible. File will not be processed. \n'
      return response
    }
  }
